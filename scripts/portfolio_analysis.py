...
def run_portfolio_analysis(processed_data_path='data/processed/adj_close.csv',
    ...
    try:
        ...
        # Ensure column names are just ticker symbols
        if isinstance(data.columns, pd.MultiIndex):
            ...
            data = data[tickers] # Ensure order and correct selection
        print(f"Historical data loaded from {processed_data_path}")
    except Exception as e:
        print(f"Error loading historical data: {e}")
        return

    # Load TSLA 12-month forecast
    ...
    try:
        ...
        # Fallback if forecast is not available
        tsla_forecast_series = data['TSLA'] # Use historical data for return calculation if forecast unavailable

    # Calculate daily returns for historical data
    daily_returns = data.pct_change().dropna()

    # --- Define Expected Returns ---
    # For TSLA: Use the mean of the LSTM 12-month future forecast
    if not tsla_forecast_series.empty and tsla_forecast_series.name == 'TSLA': # Check if it's the actual forecast
        ...
        # Fallback to historical mean if forecast loading failed or is just historical data
